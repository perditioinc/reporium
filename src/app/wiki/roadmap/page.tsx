...
export default function RoadmapPage() {
  const phases = [
    ...
    { version: '1.3.0', title: 'Gap Analysis v2 & Wiki Enhancements', status: 'done', items: ['ESSENTIAL_TOOLKIT_2026 with 10 skill areas', 'Severity tiers (missing/weak/moderate/strong)', 'Wiki skill pages with teaching content', 'WikiNavBar on all wiki pages', 'WikiRepoCard linking back to library', '?repo= and ?tag= URL params', 'GitHub Actions CI workflow'] },
    { version: '1.4.0', title: 'Intelligence Layer & Quality Gates', status: 'done', items: ['Claude-powered /intelligence/ask endpoint', 'Semantic vector search (pgvector)', 'Quality signals (has_tests, has_ci, velocity, score)', 'QualityBadge + maturity badge on repo cards', 'Knowledge graph API (/graph/edges)', 'Portfolio insights endpoint', 'Gap analysis taxonomy endpoint', 'Enrichment MCP server (18 tools, registered in Claude Code)'] },
    { version: '1.5.0', title: 'Streaming, Stacks & Graph Viz', status: 'current', items: ['AskBar SSE streaming — sources appear instantly, tokens stream in', 'Ecosystem Stacks page — 6 curated AI/ML tool combinations', 'Force-directed knowledge graph visualization at /graph', 'Security: is_private guard on all public endpoints', '/stats 500 fix (ingestion_log graceful degradation)', 'Repo card: relative updated-at + license row'] },
    { version: '1.6.0', title: 'Nightly Sync & Ecosystem Expansion', status: 'planned', items: ['reporium-db nightly sync fix', 'Automated trend snapshot cron', 'More stacks (Multimodal, Edge/Mobile, Data Science)', 'Graph viz: drag to reposition nodes', 'Ask history / saved searches'] },
  ];
